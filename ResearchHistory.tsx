...
export const ResearchHistory: React.FC<ResearchHistoryProps> = ({ language = 'English', history, onSelect, onClear }) => {
  if (history.length === 0) {
    return (
      <div className="bg-[var(--card-bg)] shadow-card  rounded-3xl p-12 text-center shadow-card border border-[var(--border-glow)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {history.map((entry) => (
          <div 
            ...
            className="bg-[var(--card-bg)] shadow-card  p-6 rounded-2xl shadow-card border border-[var(--border-glow)] hover:border-blue-300 hover:shadow-md transition cursor-pointer group"
          >
