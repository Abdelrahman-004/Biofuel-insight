    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <motion.div 
          ...
          className="bg-[#0D141A]/70 backdrop-blur-[10px] p-8 rounded-3xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/5"
        >
          ...
          <div className="space-y-4">
            {data.ImplementationEstimator?.EquipmentSetup?.map((item, i) => (
              <div key={i} className="flex items-start p-3 bg-[#0F172A] rounded-xl border border-white/5">
        ...
        >
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-slate-200 font-bold leading-relaxed">{data.ImplementationEstimator?.FeedstockRequirements}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Efficiency Adjustments</p>
              <p className="text-sm text-slate-300 leading-relaxed italic border-l-4 border-emerald-100 pl-4">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-[#0F172A] p-4 rounded-2xl">
            ...
            <p className="text-sm text-slate-300 font-bold">{data.ProductionOutput?.ByProductValueEstimation}</p>
          </div>
        ...
        >
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-slate-200 font-bold leading-relaxed">{data.ResourceRequirements?.MassBalance}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-slate-200 font-bold leading-relaxed">{data.SensitivityAnalysis?.Scenario}</p>
            </div>
      ...
      <motion.div 
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.4 }}
        className="bg-[#0F172A] p-8 rounded-3xl shadow-sm border border-white/10"
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-[#0D141A]/70 backdrop-blur-[10px] p-6 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/5">
              ...
              {[
                ...
                <div key={i} className="flex justify-between items-center py-2 border-b border-slate-50 text-sm">
                  ...
                  <div className="text-right">
                    <div className="text-white font-bold">{item.val.USD}</div>
          ...
          <div className="bg-[#0D141A]/70 backdrop-blur-[10px] p-6 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/5 flex flex-col justify-center">
            <div className="space-y-6">
              <div className="bg-[#0F172A] p-4 rounded-xl border border-white/5 flex justify-between items-center">
                ...
                <div className="text-right">
                  <div className="text-slate-300 font-black">{safeCost(finance.ContingencyBuffer).USD}</div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {data.TechnicalRiskAssessment?.ScientificChallenges?.map((challenge, i) => (
                <li key={i} className="flex items-start text-sm text-slate-300 bg-red-50/50 p-3 rounded-xl border border-red-100">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {data.TechnicalRiskAssessment?.MitigationStrategies?.map((strategy, i) => (
                <li key={i} className="flex items-start text-sm text-slate-300 bg-emerald-50/50 p-3 rounded-xl border border-emerald-100">
            ...
            {(data.TRLRoadmap || [])?.map((step, i) => (
              <div key={i} className={`flex flex-col md:flex-row items-center ${i % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                <div className="flex-1 w-full md:w-auto">
                  <div className={`p-6 rounded-2xl border ${data.ResearchInputs.TechnologyReadinessLevel >= step.trl ? 'bg-indigo-50 border-indigo-100' : 'bg-[#0F172A] border-white/5 opacity-60'}`}>
          ...
          <div className="bg-[#0D141A]/70 backdrop-blur-[10px] p-6 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/5">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center pt-4 border-t border-white/5">
                <span className="text-white font-black uppercase tracking-widest text-[10px]">Total Equipment Cost</span>
                <div className="text-right">
              ...
              <div className="flex justify-between items-center pt-2">
                <span className="text-white font-black uppercase tracking-widest text-[10px]">Installation & Setup (20-30%)</span>
                <div className="text-right">
          ...
          <div className="bg-[#0D141A]/70 backdrop-blur-[10px] p-6 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/5">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center py-2 border-b border-slate-50 text-sm">
                <span className="text-slate-400 font-medium">Laboratory Staff</span>
                <span className="text-white font-bold">{data.CostEstimation?.AnnualOperatingCost?.LaboratoryStaff || 'N/A'}</span>
              </div>
              <div className="flex justify-between items-center pt-4">
                <span className="text-white font-black uppercase tracking-widest text-[10px]">Total Annual Operating</span>
                <div className="text-right">
        ...
        >
          ...
          <ul className="space-y-3">
            {data.RiskFactors?.map((risk, i) => (
              <li key={i} className="flex items-start text-sm text-slate-300 bg-red-50/30 p-3 rounded-xl border border-red-100/50">
                <i className="fas fa-circle-exclamation text-red-500 mr-3 mt-1"></i>
                {risk}
              </li>
            ))}
        ...
        >
          ...
          <ul className="space-y-3">
            {data.Assumptions?.map((assumption, i) => (
              <li key={i} className="flex items-start text-sm text-slate-400 bg-[#0F172A] p-3 rounded-xl border border-white/5">
