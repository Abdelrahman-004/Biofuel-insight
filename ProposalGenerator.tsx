    ...
    <div className="flex flex-col lg:flex-row gap-8" dir={inputs.language === 'Arabic' ? 'rtl' : 'ltr'}>
      ...
      <div className="w-full lg:w-1/3 space-y-6">
        <motion.div 
          ...
          className="bg-[#0D141A]/70 backdrop-blur-[10px] p-6 rounded-3xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/5 hover:border-[#8B5CF6]/50 transition-all duration-300"
        >
          <h2 className="text-xl font-black text-[#8B5CF6] mb-6 flex items-center drop-shadow-[0_0_8px_rgba(139,92,246,0.5)]">
            <i className="fas fa-file-signature mx-3"></i> {inputs.language === 'Arabic' ? 'تفاصيل المقترح' : 'Proposal Details'}
          </h2>
          <form onSubmit={handleGenerate} className="space-y-4">
            
            <div className="bg-[#0F172A] p-3 rounded-xl border border-white/10 mb-4">
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">اللغة / Language</label>
              <select 
                name="language" 
                value={inputs.language} 
                onChange={handleInputChange}
                className="w-full bg-[#0D141A] border border-white/10 rounded-lg px-3 py-2 text-sm font-bold text-[#8B5CF6] outline-none shadow-sm"
              >
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-[#0F172A] border border-white/10 rounded-xl px-4 py-3 text-sm font-medium text-white focus:ring-2 focus:ring-[#10B981] focus:border-transparent transition-all outline-none placeholder:text-slate-500"
              />
            </div>
            ...
            <div>
              ...
              >
                <option value="MoHERI (Academic/Research)" className="bg-[#0F172A]">{inputs.language === 'Arabic' ? 'البحث العلمي (MoHERI)' : 'MoHERI (Academic/Research Grant)'}</option>
                <option value="PDO/OQ (Industrial/Commercial)" className="bg-[#0F172A]">{inputs.language === 'Arabic' ? 'تنمية نفط عمان / أوكيو (استثمار صناعي)' : 'PDO / OQ (Industrial Investment)'}</option>
                <option value="OTF (Startup/VC)" className="bg-[#0F172A]">{inputs.language === 'Arabic' ? 'الصندوق العماني للتكنولوجيا (شركات ناشئة)' : 'Oman Technology Fund (Startup/VC)'}</option>
              </select>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={handleInputChange}
                  className="w-full bg-[#0F172A] border border-white/10 rounded-xl px-4 py-2 text-sm font-medium text-white focus:ring-2 focus:ring-[#10B981] focus:border-transparent transition-all outline-none"
                  dir={inputs.language === 'Arabic' ? 'rtl' : 'ltr'}
                >
                  <option value="Solar Farm" className="bg-[#0F172A]">{inputs.language === 'Arabic' ? 'محطة طاقة شمسية' : 'Solar Farm'}</option>
                  <option value="Green Hydrogen" className="bg-[#0F172A]">{inputs.language === 'Arabic' ? 'هيدروجين أخضر (التحليل الكهربائي)' : 'Green Hydrogen Electrolysis'}</option>
                  <option value="Biodiesel" className="bg-[#0F172A]">{inputs.language === 'Arabic' ? 'الديزل الحيوي' : 'Biodiesel'}</option>
                  <option value="SAF" className="bg-[#0F172A]">{inputs.language === 'Arabic' ? 'وقود الطيران المستدام (SAF)' : 'SAF'}</option>
                  <option value="Waste-to-Energy" className="bg-[#0F172A]">{inputs.language === 'Arabic' ? 'تحويل النفايات إلى طاقة' : 'Waste-to-Energy'}</option>
                  <option value="Biogas" className="bg-[#0F172A]">{inputs.language === 'Arabic' ? 'الغاز الحيوي' : 'Biogas'}</option>
                </select>
              </div>
            </div>
            ...
            <button 
              ...
              className={`w-full mt-6 text-white font-bold py-4 px-6 rounded-xl transition-all shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex items-center justify-center space-x-2 ${
                status === 'GENERATING' ? 'bg-slate-800 cursor-not-allowed text-slate-500' : 'bg-gradient-to-r from-[#059669] to-[#10B981] hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] active:scale-95'
              }`}
            >
              {status === 'GENERATING' ? (
        ...
        {history.length > 0 && (
          ...
          >
            <h3 className="text-sm font-black text-white mb-4 uppercase tracking-widest">{inputs.language === 'Arabic' ? 'السجل' : 'Recent Proposals'}</h3>
            <div className="space-y-3">
              {history.map((entry) => (
                <div 
                  key={entry.id} 
                  onClick={() => { setResult(entry.fullData); setStatus('COMPLETED'); }}
                  className="p-3 bg-[#0F172A]/50 rounded-xl border border-white/5 cursor-pointer hover:bg-[#0F172A] hover:border-[#8B5CF6]/50 transition-colors"
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-bold text-slate-300 truncate pr-2">{entry.projectName}</span>
                  </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {status === 'COMPLETED' && result && (
            ...
            >
              ...
              <div id="proposal-report" className="bg-[#0D141A]/70 backdrop-blur-[10px] p-10 rounded-3xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/5 prose prose-invert max-w-none relative">
                {/* PDF BRANDING HEADER */}
                <div className="absolute top-8 left-8 right-8 flex justify-between items-start opacity-30 select-none pointer-events-none">
                  <div className="flex items-center space-x-2 grayscale">
                    ...
                    <span className="text-xl font-black tracking-tighter text-white">
                      OMAN <span className="text-[#8B5CF6]">ECOSYNC</span>
                    </span>
                  </div>
                  <div className="text-[8px] font-black tracking-[0.2em] uppercase text-right text-slate-400">
                    {inputs.language === 'Arabic' ? 'نموذج مقترح معتمد آلياً' : 'Automated Certified Proposal'} <br/>
                    {result.timestamp}
                  </div>
                </div>

                <div className="text-center mb-10 border-b border-white/10 pb-8 mt-12">
                  <h1 className="text-3xl font-black text-white mb-4">{result.title}</h1>
                  <p className="text-sm font-bold text-[#8B5CF6] uppercase tracking-widest">
                    {inputs.language === 'Arabic' ? 'أُعد خصيصاً لـ: ' : 'Prepared for: '} <span className="text-slate-300">{inputs.targetAudience}</span>
                  </p>
                  <p className="text-xs text-slate-500 mt-2">{inputs.language === 'Arabic' ? 'تاريخ الإصدار: ' : 'Generated on '}{result.timestamp}</p>
                </div>

                <div className="space-y-8">
                  <section>
                    <h2 className={`text-xl font-black text-white border-[#8B5CF6] mb-4 ${inputs.language === 'Arabic' ? 'border-r-4 pr-3' : 'border-l-4 pl-3'}`}>
                  ...
                  <section className="bg-[#0F172A] p-6 rounded-2xl border border-white/5 shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
                    <h2 className="text-xl font-black text-white mb-6 flex items-center border-b border-white/5 pb-4">
                       <i className="fas fa-chart-line mx-3 text-[#8B5CF6]"></i> {inputs.language === 'Arabic' ? 'الجدوى المالية والعوائد' : 'Financial Viability & ROI'}
                    </h2>
                    
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                      <div className="bg-[#0D141A] p-4 rounded-xl border border-white/5">
                        <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'رأس المال (CAPEX)' : 'Total CAPEX'}</div>
                        <div className="text-lg font-bold text-[#8B5CF6]">{result.financials.totalCapex}</div>
                      </div>
                      <div className="bg-[#0D141A] p-4 rounded-xl border border-white/5">
                        <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'التشغيل (OPEX)' : 'Annual OPEX'}</div>
                        <div className="text-lg font-bold text-amber-400">{result.financials.annualOpex}</div>
                      </div>
                      <div className="bg-[#0D141A] p-4 rounded-xl border border-white/5">
                        <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'العائد (ROI)' : 'ROI Percentage'}</div>
                        <div className="text-lg font-bold text-white">{result.financials.roiPercentage}</div>
                      </div>
                      <div className="bg-[#0D141A] p-4 rounded-xl border border-white/5">
                        <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'فترة الاسترداد' : 'Payback Period'}</div>
                  ...
                  <section className="bg-blue-900/20 p-6 rounded-2xl border border-blue-500/20">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div className="bg-[#0F172A] p-4 rounded-xl border border-white/5 shadow-sm">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'توفير الكربون السنوي' : 'Estimated Annual CO2 Savings'}</p>
                        <p className="text-lg font-black text-blue-400">{result.carbonCreditPotential.estimatedTonsSaved}</p>
                      </div>
                      <div className="bg-[#0F172A] p-4 rounded-xl border border-white/5 shadow-sm">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'العائد الإضافي المحتمل' : 'Potential Monetary Value'}</p>
                        <p className="text-lg font-black text-[#8B5CF6]">{result.carbonCreditPotential.monetaryValueRange}</p>
                      </div>
                    </div>
                    <p className="text-sm text-slate-300 leading-relaxed">{result.carbonCreditPotential.explanation}</p>
                  </section>
                  ...
                  <section>
                    ...
                    <p className="text-slate-300 leading-relaxed font-medium">{result.conclusion}</p>
                  </section>
                </div>
              </div>
            </motion.div>
