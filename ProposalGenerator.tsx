      ...
      <div className="w-full lg:w-1/3 space-y-6">
        ...
        >
          ...
          <form onSubmit={handleGenerate} className="space-y-4">
            ...
            <div>
              <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'اسم المشروع' : 'Project Name'}</label>
              <input 
                type="text" 
                name="projectName" 
                required
            ...
            <div>
              <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'الجهة المانحة / المستثمر' : 'Target Audience'}</label>
              <select 
                name="targetAudience" 
              ...
              >
                <option value="MoHERI (Academic/Research)" className="bg-[var(--bg-main)]">{inputs.language === 'Arabic' ? 'البحث العلمي (MoHERI)' : 'MoHERI (Academic/Research Grant)'}</option>
                <option value="PDO/OQ (Industrial/Commercial)" className="bg-[var(--bg-main)]">{inputs.language === 'Arabic' ? 'تنمية نفط عمان / أوكيو (استثمار صناعي)' : 'PDO / OQ (Industrial Investment)'}</option>
                <option value="OTF (Startup/VC)" className="bg-[var(--bg-main)]">{inputs.language === 'Arabic' ? 'الصندوق العماني للتكنولوجيا (شركات ناشئة)' : 'Oman Technology Fund (Startup/VC)'}</option>
              </select>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'المادة الخام' : 'Feedstock'}</label>
                <input 
                  type="text" 
                  name="feedstock" 
                  required
              ...
              <div>
                <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'نوع المشروع/الوقود' : 'Project / Energy Type'}</label>
                <select 
                  name="biofuelType" 
                  ...
                  dir={inputs.language === 'Arabic' ? 'rtl' : 'ltr'}
                >
                  <option value="Solar Farm" className="bg-[var(--bg-main)]">{inputs.language === 'Arabic' ? 'محطة طاقة شمسية' : 'Solar Farm'}</option>
                  <option value="Green Hydrogen" className="bg-[var(--bg-main)]">{inputs.language === 'Arabic' ? 'هيدروجين أخضر (التحليل الكهربائي)' : 'Green Hydrogen Electrolysis'}</option>
                  <option value="Biodiesel" className="bg-[var(--bg-main)]">{inputs.language === 'Arabic' ? 'الديزل الحيوي' : 'Biodiesel'}</option>
                  <option value="SAF" className="bg-[var(--bg-main)]">{inputs.language === 'Arabic' ? 'وقود الطيران المستدام (SAF)' : 'SAF'}</option>
                  <option value="Waste-to-Energy" className="bg-[var(--bg-main)]">{inputs.language === 'Arabic' ? 'تحويل النفايات إلى طاقة' : 'Waste-to-Energy'}</option>
                  <option value="Biogas" className="bg-[var(--bg-main)]">{inputs.language === 'Arabic' ? 'الغاز الحيوي' : 'Biogas'}</option>
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'حجم الإنتاج' : 'Target Capacity'}</label>
                <input 
                  type="text" 
                  name="capacity" 
                  required
              ...
              <div>
                <label className="block text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'الميزانية' : 'Estimated Budget'}</label>
                <input 
                  type="text" 
                  name="budget" 
                  required
        ...
        {history.length > 0 && (
          ...
          >
            <h3 className="text-sm font-black text-[var(--text-primary)] mb-4 uppercase tracking-widest">{inputs.language === 'Arabic' ? 'السجل' : 'Recent Proposals'}</h3>
        ...
        <AnimatePresence mode="wait">
          ...
          {status === 'GENERATING' && (
            ...
            >
              ...
              <p className="text-[var(--text-secondary)]">{inputs.language === 'Arabic' ? 'يتم التوافق مع رؤية عمان 2040 وبناء الجداول المالية.' : 'Aligning with Oman Vision 2040 and structuring arguments.'}</p>
            </motion.div>
          )}
              ...
              <div id="proposal-report" className="bg-[var(--card-bg)]/90 shadow-card backdrop-blur-[10px] p-10 rounded-3xl  border border-[var(--border-glow)] prose prose-invert max-w-none relative">
                ...
                <div className="absolute top-8 left-8 right-8 flex justify-between items-start opacity-30 select-none pointer-events-none">
                  <div className="flex items-center space-x-2 grayscale">
                    ...
                    <span className="text-xl font-black tracking-tighter text-[var(--text-primary)]">
                      {inputs.language === 'Arabic' ? <>عُمَان <span className="text-[#8B5CF6]">إيكوسينك</span></> : <>OMAN <span className="text-[#8B5CF6]">ECOSYNC</span></>}
                    </span>
                  </div>
                ...
                <div className="text-center mb-10 border-b border-[var(--border-glow)] pb-8 mt-12">
                  ...
                  <p className="text-sm font-bold text-[#8B5CF6] uppercase tracking-widest">
                    {inputs.language === 'Arabic' ? 'أُعد خصيصاً لـ: ' : 'Prepared for: '} <span className="text-[var(--text-secondary)]">{inputs.targetAudience}</span>
                  </p>
                  <p className="text-xs text-[var(--text-secondary)] mt-2">{inputs.language === 'Arabic' ? 'تاريخ الإصدار: ' : 'Generated on '}{result.timestamp}</p>
                </div>

                <div className="space-y-8">
                  <section>
                    <h2 className={`text-xl font-black text-[var(--text-primary)] border-[#8B5CF6] mb-4 ${inputs.language === 'Arabic' ? 'border-r-4 pr-3' : 'border-l-4 pl-3'}`}>
                  ...
                  <section className="bg-[var(--bg-main)] p-6 rounded-2xl border border-[var(--border-glow)] ">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                      <div className="bg-[var(--card-bg)] shadow-card p-4 rounded-xl border border-[var(--border-glow)]">
                        <div className="text-[10px] text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'رأس المال (CAPEX)' : 'Total CAPEX'}</div>
                        <div className="text-lg font-bold text-[#8B5CF6]">{result.financials.totalCapex}</div>
                      </div>
                      <div className="bg-[var(--card-bg)] shadow-card p-4 rounded-xl border border-[var(--border-glow)]">
                        <div className="text-[10px] text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'التشغيل (OPEX)' : 'Annual OPEX'}</div>
                        <div className="text-lg font-bold text-amber-400">{result.financials.annualOpex}</div>
                      </div>
                      <div className="bg-[var(--card-bg)] shadow-card p-4 rounded-xl border border-[var(--border-glow)]">
                        <div className="text-[10px] text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'العائد (ROI)' : 'ROI Percentage'}</div>
                        <div className="text-lg font-bold text-[var(--text-primary)]">{result.financials.roiPercentage}</div>
                      </div>
                      <div className="bg-[var(--card-bg)] shadow-card p-4 rounded-xl border border-[var(--border-glow)]">
                        <div className="text-[10px] text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'فترة الاسترداد' : 'Payback Period'}</div>
                        <div className="text-lg font-bold text-[var(--text-primary)]">{result.financials.paybackPeriod}</div>
                      </div>
                    </div>

                    <div className="mb-4">
                      <h3 className="text-sm font-bold text-[var(--text-secondary)] mb-3">{inputs.language === 'Arabic' ? 'خطة وجدول استرداد التمويل للمستثمر:' : 'Investor Return & Installment Schedule:'}</h3>
                          ...
                          <tbody className="divide-y divide-slate-700/50">
                            {result.financials.installmentSchedule.map((inst, i) => (
                              <tr key={i} className="hover:bg-[var(--bg-main)]/50 transition-colors">
                                <td className={`p-3 font-semibold text-[var(--accent-emerald)] dark:text-emerald-400 ${inputs.language === 'Arabic' ? 'text-right' : 'text-left'}`}>{inst.period}</td>
                                <td className={`p-3 font-bold text-[var(--text-primary)] ${inputs.language === 'Arabic' ? 'text-right' : 'text-left'}`}>{inst.paymentAmount}</td>
                                <td className={`p-3 text-[var(--text-secondary)] ${inputs.language === 'Arabic' ? 'text-right' : 'text-left'}`}>{inst.milestoneDescription}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                  ...
                  <section className="bg-blue-900/20 p-6 rounded-2xl border border-blue-500/20">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div className="bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-glow)] shadow-sm">
                        <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'توفير الكربون السنوي' : 'Estimated Annual CO2 Savings'}</p>
                        <p className="text-lg font-black text-blue-400">{result.carbonCreditPotential.estimatedTonsSaved}</p>
                      </div>
                      <div className="bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-glow)] shadow-sm">
                        <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mb-1">{inputs.language === 'Arabic' ? 'العائد الإضافي المحتمل' : 'Potential Monetary Value'}</p>
